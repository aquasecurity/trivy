...
Trivy locates the registry API through Terraform's [remote service discovery](https://developer.hashicorp.com/terraform/internals/remote-service-discovery): it reads `https://<hostname>/.well-known/terraform.json` and uses the advertised `modules.v1` endpoint, so registries that serve the module API under a custom path (for example HCP Terraform at `/api/registry/v1/modules/`) work without extra configuration. Hosts that do not publish a discovery document fall back to `https://<hostname>/v1/modules/`. A relative download location returned by the registry (as GitLab does) is resolved against the download endpoint, as the [module registry protocol](https://developer.hashicorp.com/terraform/internals/module-registry-protocol#download-source-code-for-a-specific-module-version) allows.
