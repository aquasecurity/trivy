...
RapidFort publishes hardened builds of [Ubuntu](../os/ubuntu.md), [Alpine](../os/alpine.md), and [Red Hat Enterprise Linux](../os/rhel.md)-based container images together with the corresponding security advisories.
...
Because RapidFort curates its own advisories for the packages it ships (including third-party binaries such as `rf-` prefixed builds), Trivy scans those packages against the RapidFort feed instead of skipping them as it would under a standard base OS scan.
