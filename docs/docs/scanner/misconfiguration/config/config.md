...
```tf
#trivy:ignore:s3-enable-logging:exp:2024-03-10
resource "aws_s3_bucket" "example" {
  bucket = "test"
}
```

The `s3-enable-logging` check will be ignored until `2024-03-10` until the ignore rule expires.
...
#trivy:ignore:ec2-no-public-ingress-sgr[from_port=3306]
...
The `ec2-no-public-ingress-sgr` check will be ignored only for the `aws_security_group_rule` resource with port number `5432`. It is important to note that the ignore rule should not enclose the attribute value in quotes, despite the fact that the port is represented as a string.

If you want to ignore multiple resources on different attributes, you can specify multiple ignore rules:

```tf
#trivy:ignore:ec2-no-public-ingress-sgr[from_port=3306]
#trivy:ignore:ec2-no-public-ingress-sgr[from_port=5432]
```
...
#trivy:ignore:ec2-no-public-ingress-sgr[from_port=1000,type=egress]
resource "aws_security_group_rule" "example" {
...
```tf
#trivy:ignore:s3-enable-logging
module "s3_bucket" {
