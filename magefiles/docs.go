...
func main() {
	...
	if err := updateTelemetryDocs("./docs/docs/advanced/telemetry.md", allFlagGroups); err != nil {
		log.Fatal("Fatal error in telemetry docs generation", log.Err(err))
	}
}

// updateTelemetryDocs updates the telemetry section in the documentation file
// with the flags that are safe to be included in telemetry.
func updateTelemetryDocs(filename string, allFlagGroups []flag.FlagGroup) error {
	...
	endIndex := strings.Index(string(content), "<!-- telemetry end -->")

	if startIndex == -1 || endIndex == -1 || startIndex >= endIndex {
		return fmt.Errorf("telemetry section not found in %s", filename)
	}
