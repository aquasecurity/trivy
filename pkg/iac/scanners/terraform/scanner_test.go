...
`
	...
	for _, result := range failed {
		// public access block
		assert.NotEqual(t, "AWS-0094", result.Rule().ID, "AWS-0094 should not be reported - was found at "+result.Metadata().Range().String())
		// encryption
		assert.NotEqual(t, "AWS-0088", result.Rule().ID)
		// logging
		assert.NotEqual(t, "AWS-0089", result.Rule().ID)
		// versioning
		assert.NotEqual(t, "AWS-0090", result.Rule().ID)
	}
}
...
`
	...
	for _, result := range results.GetFailed() {
		// public access block
		assert.NotEqual(t, "AWS-0094", result.Rule().ID)
	}

}

// PoC for replacing Go with Rego: AWS-0001
