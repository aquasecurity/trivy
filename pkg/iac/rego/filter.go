...
func FrameworksFilter(frameworks []framework.Framework) RegoModuleFilter {
	return func(module *ast.Module, metadata *StaticMetadata) bool {
		return metadata.hasAnyFramework(frameworks)
	}
}

// SkipDeprecatedFilter returns a filter that excludes deprecated modules,
// if the skip parameter is enabled.
func SkipDeprecatedFilter(skip bool) RegoModuleFilter {
	return func(module *ast.Module, metadata *StaticMetadata) bool {
		if !skip {
			return true
		}
