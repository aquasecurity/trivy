...
func (r *Result) OverrideMetadata(metadata defsecTypes.MisconfigMetadata) {
	r.metadata = metadata
}
...
func (r Result) Metadata() defsecTypes.MisconfigMetadata {
	return r.metadata
}
...
func (r *Result) AbsolutePath(fsRoot string, metadata defsecTypes.MisconfigMetadata) string {
...
func (r *Result) RelativePathTo(fsRoot, to string, metadata defsecTypes.MisconfigMetadata) string {
...
type MetadataProvider interface {
	GetMetadata() defsecTypes.MisconfigMetadata
...
func getMetadataFromSource(source interface{}) defsecTypes.MisconfigMetadata {
	...
	return metaVal.Interface().(defsecTypes.MisconfigMetadata)
}
...
func (r *Results) SetSourceAndFilesystem(source string, f fs.FS, logicalSource bool) {
	for i := range *r {
		...
		switch {
		case m.IsExplicit():
			m = defsecTypes.NewExplicitMisconfigMetadata(newrng, m.Reference())
		default:
			m = defsecTypes.NewMisconfigMetadata(newrng, m.Reference())
		}
		if parent != nil {
