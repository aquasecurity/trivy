...
func describeCertificate(
	...
	oid, err := signatureAlgorithmOID(cert)
	if err != nil {
		return nil, err
	}
	...
	signature := crypto.DescribeAlgorithm(oid, 0, "")
	...
	return append([]ftypes.CryptoAssetInfo{certificate}, related...), nil
}
...
func signatureAlgorithmOID(cert *stdx509.Certificate) (string, error) {
	...
	if _, err := asn1.Unmarshal(cert.Raw, &certificate); err != nil {
		return "", xerrors.Errorf("unmarshal certificate: %w", err)
	}
	return certificate.SignatureAlgorithm.Algorithm.String(), nil
}
