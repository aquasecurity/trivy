...
// through its public projection, so it differs from a public key only in its key type. The
// format and the encoding stay empty for a key that has no container of its own, such as
// the key carried by a certificate.
//
...
func DescribeKey(
...
) (key, algorithm ftypes.CryptoAssetInfo, err error) {
	...
	algorithm = DescribeAlgorithm(algorithmOID, size, curve)

	key = ftypes.CryptoAssetInfo{
		Kind:     ftypes.CryptoKindKey,
		KeyType:  keyType,
		Identity: identity,
		Name:     algorithm.Name + " " + string(keyType) + " key",
...
func publicKeyInfo(pub stdcrypto.PublicKey) (ftypes.CryptoIdentity, string, error) {
	...
	if err != nil {
		return ftypes.CryptoIdentity{}, "", xerrors.Errorf("%s: %w", err, ErrNoCanonicalForm)
	}
