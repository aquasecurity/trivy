...
func Test_cryptoAnalyzer_Analyze(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// The key of the certificate and the standalone key are the same key stored
			// twice, so the file reports it once without a container and once with one.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// An identity may repeat within a file when the material is stored twice, so
			// what has to stay unique is the identity together with the container.
			...
			occurrences := set.New[occurrence]()
			...
			assert.Equal(t, len(got.CryptoAssets), occurrences.Size(), "assets repeat an occurrence")
...
// sizedFileInfo reports a size, which is all Required reads from os.FileInfo.
...
func Test_cryptoAnalyzer_Required(t *testing.T) {
	tests := []struct {
		...
		{name: "extensionless", filePath: "certificates/server"},
		{
			...
			info:     sizedFileInfo{size: maxFileSize},
			want:     true,
		},
		{
			name:     "eligible extension above the size limit",
			filePath: "certificates/server.key",
			info:     sizedFileInfo{size: maxFileSize + 1},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			a := &cryptoAnalyzer{}
			assert.Equal(t, tt.want, a.Required(tt.filePath, tt.info))
		})
	}
}
