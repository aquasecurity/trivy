...
func TestResolveDependency(t *testing.T) {
	var tests = map[string]struct {
		...
		"low": {
			...
			want:                set.New("libblkid", "libuuid", "musl"),
		},
		"medium": {
			pkgName:             "libgcab",
			apkIndexArchivePath: "testdata/history_v3.9.json",
			want: set.New("busybox", "libblkid", "libuuid", "musl", "libmount", "pcre", "glib",
				"libgcab", "libintl", "zlib", "libffi"),
		},
		"high": {
			pkgName:             "postgresql",
			apkIndexArchivePath: "testdata/history_v3.9.json",
			want: set.New("busybox", "ncurses-terminfo-base", "ncurses-terminfo", "libedit", "db", "libsasl",
				"libldap", "libpq", "postgresql-client", "tzdata", "libxml2", "postgresql", "musl", "libcrypto1.1",
				"libssl1.1", "ncurses-libs", "zlib"),
		},
		"package alias": {
			pkgName:             "sqlite-dev",
			apkIndexArchivePath: "testdata/history_v3.9.json",
		...
		"circular dependencies": {
			...
			want: set.New("busybox", "c-ares", "ca-certificates", "http-parser", "libcrypto1.0", "libgcc",
				"libressl2.6-libcrypto", "libssl1.0", "libstdc++", "libuv", "musl", "nodejs", "nodejs-npm", "zlib"),
		},
	}
	analyzer := alpineCmdAnalyzer{}
	for testName, v := range tests {
