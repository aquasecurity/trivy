...
func (a cargoAnalyzer) parseRootCargoTOML(fsys fs.FS, filePath string) (string, []string, map[string]string, error) {
	...
	for _, member := range members {
		...
		memberPkg, memberDeps, _, _, err := a.parseCargoTOML(fsys, memberPath, rootVersion)
		if err != nil {
...
func (a cargoAnalyzer) parseCargoTOML(fsys fs.FS, filePath string, workspaceVersion string) (string, Dependencies, []string, string, error) {
	// Parse Cargo.toml
	...
	case string:
		tomlFile.Package.Version = tomlFile.Package.Version
