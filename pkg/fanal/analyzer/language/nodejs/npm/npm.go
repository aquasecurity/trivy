...
func (a npmLibraryAnalyzer) PostAnalyze(_ context.Context, input analyzer.PostAnalysisInput) (*analyzer.AnalysisResult, error) {
	...
	err := fsutils.WalkDir(input.FS, ".", required, func(filePath string, d fs.DirEntry, r dio.ReadSeekerAt) error {
		...
		app, err := a.parseNpmPkgLock(input.FS, filePath, input.Options.IncludeDev)
		if err != nil {
...
func (a npmLibraryAnalyzer) parseNpmPkgLock(fsys fs.FS, path string, includeDev bool) (*types.Application, error) {
	...
	return language.Parse(types.Npm, path, includeDev, file, a.lockParser)
}
