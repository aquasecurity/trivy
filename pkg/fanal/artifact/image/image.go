...
func (a Artifact) inspectLayer(ctx context.Context, layerInfo LayerInfo, disabled []analyzer.Type) (types.BlobInfo, error) {
	...
	cl, err := a.uncompressedLayer(layerInfo.DiffID)
	if err != nil {
	...
	// Walk a tar layer
	opqDirs, whFiles, err := a.walker.Walk(cl, func(filePath string, info os.FileInfo, opener analyzer.Opener) error {
...
func (a Artifact) uncompressedLayer(diffID string) (*cachedLayer, error) {
	...
	cl, err := newCachedLayer(rc, size, a.artifactOption.Slow, digest)
	if err != nil {
