...
var builtinRules = []Rule{
	{
		...
		Regex:               MustCompile(fmt.Sprintf(`(?P<secret>(A3T[A-Z0-9]|AKIA|AGPA|AIDA|AROA|AIPA|ANPA|ANVA|ASIA)[A-Z0-9]{16})%s%s`, quote, endSecret)),
	...
	},
	{
		...
		Severity:            "CRITICAL",
		Regex:               MustCompile(`(?P<secret>ghp_[0-9a-zA-Z]{36})`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
	...
	},
	{
		...
		Severity:            "CRITICAL",
		Regex:               MustCompile(`(?P<secret>gho_[0-9a-zA-Z]{36})`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
	...
	},
	{
		...
		Severity:            "CRITICAL",
		Regex:               MustCompile(`(?P<secret>ghr_[0-9a-zA-Z]{76})`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
	...
	},
	{
		...
		Severity:            "CRITICAL",
		Regex:               MustCompile(`(?P<secret>glpat-[0-9a-zA-Z\-\_]{20})`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
	...
	},
	{
		...
		Regex:               MustCompile(fmt.Sprintf(`(?P<secret>hf_[A-Za-z0-9]{34,40})%s`, endWord)),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
		Keywords:            []string{"hf_"},
	},
	...
	},
	{
		...
		Title:               "Slack token",
		Severity:            "HIGH",
		Regex:               MustCompile(`(?P<secret>xox[baprs]-([0-9a-zA-Z]{10,48}))`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
	...
	},
	{
		...
		Severity:            "LOW",
		Regex:               MustCompile(`(?P<secret>(?i)pk_(test|live)_[0-9a-z]{10,32})`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
	...
	},
	{
		...
		Severity:            "CRITICAL",
		Regex:               MustCompile(`(?P<secret>(?i)sk_(test|live)_[0-9a-z]{10,32})`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
	...
	},
	{
		...
		Severity:            "MEDIUM",
		Regex:               MustCompile(`(?P<secret>FLW(PUB|SEC)K_TEST-(?i)[a-h0-9]{32}-X)`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
	...
	},
	{
		...
		Severity:            "MEDIUM",
		Regex:               MustCompile(`(?P<secret>FLWSECK_TEST[a-h0-9]{12})`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
	...
	// this keeps a typed key from also matching the bare legacy pattern.
	{
		...
		Severity:            "CRITICAL",
		Regex:               MustCompile(`(?P<secret>sk-proj-[A-Za-z0-9_-]{20,100}T3BlbkFJ[A-Za-z0-9_-]{20,100})`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
	...
	},
	{
		...
		Severity:            "CRITICAL",
		Regex:               MustCompile(`(?P<secret>sk-svcacct-[A-Za-z0-9_-]{20,100}T3BlbkFJ[A-Za-z0-9_-]{20,100})`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
		Keywords:            []string{"T3BlbkFJ"},
	},
	{
		...
		Severity:            "CRITICAL",
		Regex:               MustCompile(`(?P<secret>sk-admin-[A-Za-z0-9_-]{20,100}T3BlbkFJ[A-Za-z0-9_-]{20,100})`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
		Keywords:            []string{"T3BlbkFJ"},
	},
	{
		...
		Severity:            "HIGH",
		Regex:               MustCompile(`(?P<secret>sk-(?:None-)?[A-Za-z0-9]{20,42}T3BlbkFJ[A-Za-z0-9]{20,42})`),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
		Keywords:            []string{"T3BlbkFJ"},
	},
	...
	},
	{
		...
		Severity:            "MEDIUM",
		Regex:               MustCompile(fmt.Sprintf(`(?P<secret>ek_[a-f0-9]{32})%s`, endWord)),
		LeadingWordBoundary: true,
		SecretGroupName:     "secret",
		Keywords:            []string{"ek_"},
	},
