...
func TestContainerd_SearchLocalStoreByNameOrDigest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			img, cleanup, err := image.NewContainerImage(ctx, tt.searchName, types.ImageOptions{}, runtimes)
			defer cleanup()
			if tt.expectErr {
				require.Error(t, err)
				return
...
func localImageTestWithNamespace(t *testing.T, namespace string) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			runtimes, err := image.WithRuntimes(types.Runtimes{types.ContainerdRuntime})
			require.NoError(t, err)
			img, cleanup, err := image.NewContainerImage(ctx, tt.imageName, types.ImageOptions{}, runtimes)
			require.NoError(t, err)
			defer cleanup()
