...
func TestFanal_Library_DockerLessMode(t *testing.T) {
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			img, cleanup, err := image.NewContainerImage(ctx, tt.remoteImageName, types.ImageOptions{Runtimes: types.Runtimes{types.RemoteRuntime}})
			require.NoError(t, err)
			defer cleanup()
...
func TestFanal_Library_DockerMode(t *testing.T) {
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			img, cleanup, err := image.NewContainerImage(ctx, tt.remoteImageName, types.ImageOptions{Runtimes: types.Runtimes{types.DockerRuntime}})
			require.NoError(t, err, tt.name)
			defer cleanup()
