...
func (m *Marshaler) cryptoProperties(asset *ftypes.CryptoAssetInfo) *cdx.CryptoProperties {
	...
	m.logger.Debug("Unable to convert cryptographic asset to CycloneDX format", log.Any("kind", asset.Kind))
	return nil
}
...
func (m *Marshaler) certificateProperties(asset *ftypes.CryptoAssetInfo) *cdx.CertificateProperties {
	...
	return &cdx.CertificateProperties{
		...
		// "excluding it's signature", which is a digest the model does not keep.
		// TODO: clarify the intended value with the CycloneDX specification.
...
func (m *Marshaler) relatedCryptoMaterialProperties(asset *ftypes.CryptoAssetInfo) *cdx.RelatedCryptoMaterialProperties {
	...
	// An encrypted container is identified by the digest of the container, which says
	// nothing about the key inside it.
	if asset.Identity.Method == ftypes.CryptoMethodSPKISHA256 {
...
// relatedCryptographicAssets refers to the neighbouring assets. CycloneDX describes what
...
// identity, so one component has an occurrence per file it was found in, each tagged with
// its own layer.
func (*Marshaler) evidence(occurrences []core.Occurrence) *cdx.Evidence {
	...
	found := xslices.Map(occurrences, func(occurrence core.Occurrence) cdx.EvidenceOccurrence {
		return cdx.EvidenceOccurrence{
			Location:          occurrence.Location,
			AdditionalContext: occurrence.Layer.DiffID,
		}
	})
