...
func TestUnmarshaler_Unmarshal(t *testing.T) {
	tests := []struct {
		...
		{
			// Trivy no longer outputs the `aquasecurity:trivy:FilePath` property (`evidence.occurrences` is used instead),
			// but SBOMs generated by older versions of Trivy contain this property (see `group-in-name.json`).
			name:      "happy path for jar with evidence.occurrences",
			inputFile: "testdata/happy/evidence-occurrences.json",
			want: types.SBOM{
				Applications: []ftypes.Application{
					{
						Type: "jar",
