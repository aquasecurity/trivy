...
func TestMarshaler_MarshalCryptoAssets(t *testing.T) {
	tests := []struct {
			...
			assets: []ftypes.CryptoAsset{
				{
					...
					Layer:    ftypes.Layer{DiffID: "sha256:base"},
				},
			},
			want: []cdx.Component{
				{
			...
			want: []cdx.Component{
				{
					...
					CryptoProperties: &cdx.CryptoProperties{
						...
						RelatedCryptoMaterialProperties: &cdx.RelatedCryptoMaterialProperties{
							...
							// The digest identifies the encrypted container, so it is not
							// reported as the fingerprint of the key inside it.
							Format: "PKCS#8",
						},
					},
					Evidence: &cdx.Evidence{
		...
		{
			name: "one asset found in several files and layers",
			assets: []ftypes.CryptoAsset{
				{
					CryptoAssetInfo: ftypes.CryptoAssetInfo{
						Kind: ftypes.CryptoKindAlgorithm,
				...
				},
				{
					...
					Layer:    ftypes.Layer{DiffID: "sha256:user"},
				},
			},
			want: []cdx.Component{
				{
...
// TestMarshaler_MarshalCryptoAssetReferences checks that every reference between
// cryptographic assets names a component of the same document. The references are written
// as strings and are not resolved through the components, so nothing else keeps them
