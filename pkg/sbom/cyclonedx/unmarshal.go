...
import (
	...
	version "github.com/aquasecurity/go-version/pkg/semver"
...
const (
	PropertyK8sResource = "resource:Name"
)
...
func (c *BOM) parseSBOM(bom *cdx.BOM) error {
	...
	for bomRef := range c.dependencies {
		...
		case cdx.ComponentTypeApplication: // It would be a lock file in a CycloneDX report generated by Trivy
			if core.LookupProperty(component.Properties, PropertyK8sResource) != "" {
				app, pkgs := c.parseK8sPkgs(component)
	...
	if len(c.Packages) == 0 && len(pkgInfos) != 0 {
		if !c.OS.Detected() {
			...
			c.Packages = append(c.Packages, pkgInfos...)
		}
	}
...
func (c *BOM) parseK8sPkgs(component cdx.Component) ([]ftypes.Application, []ftypes.PackageInfo) {
	simpleVersaion, err := version.Parse(component.Version)
	// Avoid false positive on external and cloud provider versions example: kube-proxy:v1.25.4-hotfix.20221216 or 1.24.10-gke.2300
	// k8s-vuln-db support only upstream official versions
	if err != nil || len(simpleVersaion.PreRelease()) != 0 {
		return []ftypes.Application{}, []ftypes.PackageInfo{}
	}

	packages := []ftypes.Package{
		{
			Name:    component.Name,
			Version: simpleVersaion.String(),
		}}
