...
func InitBuiltinPolicies(ctx context.Context, cacheDir string, quiet, skipUpdate bool, policyBundleRepository string, registryOptions ftypes.RegistryOptions) ([]string, error) {
	mu.Lock()
	defer mu.Unlock()
	...
	needsUpdate := false
	if !skipUpdate {
		needsUpdate, err = client.NeedsUpdate(ctx, registryOptions)
		if err != nil {
	...
	if needsUpdate {
		...
		if err = client.DownloadBuiltinPolicies(ctx, registryOptions); err != nil {
