  ...
  $ trivy server`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if globalOptions.ShowVersion {
				...
				showVersion(globalOptions.CacheDir, versionFormat, version)
			} else {
				return cmd.Help()
			}
			return nil
...
func NewVersionCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			showVersion(options.CacheDir, versionFormat, cmd.Version)
...
func showVersion(cacheDir, outputFormat, version string) {
