...
func (r regoResult) GetMetadata() defsecTypes.MisconfigMetadata {
	var m defsecTypes.MisconfigMetadata
	if !r.Managed {
		m = defsecTypes.NewUnmanagedMisconfigMetadata()
	} else {
		rng := defsecTypes.NewRangeWithFSKey(r.Filepath, r.StartLine, r.EndLine, r.SourcePrefix, r.FSKey, r.FS)
		if r.Explicit {
			m = defsecTypes.NewExplicitMisconfigMetadata(rng, r.Resource)
		} else {
			m = defsecTypes.NewMisconfigMetadata(rng, r.Resource)
		}
	}
	if r.Parent != nil {
