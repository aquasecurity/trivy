...
func Test_EmbeddedLoading(t *testing.T) {
	...
	assert.True(t, found, "no embedded rego policies were registered as trules")
}
...
deny[res]{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			newRule, err := ast.ParseModuleWithOpts("/trules/newrule.rego", tc.inputPolicy, ast.ParserOptions{
				ProcessAnnotation: true,
			})
			require.NoError(t, err)

			policies["/trules/newrule.rego"] = newRule
			switch {
