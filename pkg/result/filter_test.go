...
func TestFilter(t *testing.T) {
	...
	tests := []struct {
				...
				Results: []types.Result{
					{
						...
						IgnoredVulnerabilities: []types.DetectedVulnerability{
							{
								VulnerabilityID:  "CVE-2019-0001",
								PkgName:          "foo",
								InstalledVersion: "1.2.3",
		...
		{
			name: "filter by VEX - show ignored",
			args: args{
				report: types.Report{
					Results: types.Results{
						types.Result{
		...
		{
			name: "ignore unfixed - show ignored",
			args: args{
				report: types.Report{
					Results: types.Results{
						types.Result{
		...
		{
			name: "ignore file for misconf - show ignored",
			args: args{
				report: types.Report{
					Results: types.Results{
						{
		...
		{
			name: "happy path with duplicates, one with empty fixed version - show ignored",
			args: args{
				report: types.Report{
					Results: types.Results{
						{
		...
		{
			name: "happy path with duplicates and different package paths - show ignored",
			args: args{
				report: types.Report{
					Results: types.Results{
						{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := result.Filter(context.Background(), tt.args.report, result.FilterOption{
				...
				ShowIgnored:    tt.args.showIgnored,
