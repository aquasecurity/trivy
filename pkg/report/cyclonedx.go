...
type CycloneDXWriter struct {
	Output io.Writer
	Option CycloneDXOption
}

// CycloneDXOption holds the options for output CycloneDX
type CycloneDXOption struct {
	AppVendor  string
	AppName    string
	AppVersion string
}
...
func (cw CycloneDXWriter) Write(report Report) error {
	bom, err := convertToBom(report, cw.Option)
	if err != nil {
...
func convertToBom(r Report, option CycloneDXOption) (*cdx.BOM, error) {
	bom := cdx.NewBOM()
	bom.Metadata = &cdx.Metadata{
		Timestamp: time.Now().Format(time.RFC3339),
		Tools: &[]cdx.Tool{
			{
				Vendor:  option.AppVendor,
				Name:    option.AppName,
