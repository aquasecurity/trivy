...
func run() error {
	...
	// All "not_affected" statements across Trivy versions
	notAffectedVulns := map[UniqueKey][]vex.Statement{}

	// Indicate one or more Trivy versions are affected by the vulnerability.
	// This means that the version cannot be omitted.
	affectedVulns := map[UniqueKey]struct{}{}
...
func processTag(ctx context.Context, tag string) (map[UniqueKey]vex.Statement, map[UniqueKey]struct{}, error) {
	...
	product.Version = tag
	notAffected := map[UniqueKey]vex.Statement{}
	affected := map[UniqueKey]struct{}{}
...
func combineDocs(notAffected map[UniqueKey][]vex.Statement, affected map[UniqueKey]struct{}) []vex.Statement {
	...
	statements := map[UniqueKey]vex.Statement{}
